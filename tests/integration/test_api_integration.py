...
class TestAPIIntegration:
    ...
    def test_sync_endpoint_integration(
        self, mock_analyze, mock_aws_services, sync_event, lambda_context
    ):
    ...
    def test_async_endpoint_integration(
        self, mock_aws_services, async_event, lambda_context
    ):
        ...
        try:
            s3.get_object(
                Bucket="test-aurastream-documents", Key=f"documents/{job_id}.txt"
            )
        except s3.exceptions.NoSuchKey:
            pytest.fail("Document was not stored in S3")

    def test_status_endpoint_integration(
        self, mock_aws_services, status_event, lambda_context
    ):
    ...
    def test_sync_endpoint_validation_error(self, mock_aws_services, lambda_context):
        ...
        invalid_event = {
            "httpMethod": "POST",
            "path": "/analyze/sync",
            "body": json.dumps({"text": ""}),  # Empty text should fail validation
            "headers": {"Content-Type": "application/json"},
            "requestContext": {"requestId": "test-request-id"},
        }
    ...
    def test_status_endpoint_validation_error(self, mock_aws_services, lambda_context):
        ...
        invalid_event = {
            ...
            "pathParameters": {"job_id": "invalid-job-id"},  # Not a valid UUID
            "headers": {"Content-Type": "application/json"},
            "requestContext": {"requestId": "test-request-id"},
        }
